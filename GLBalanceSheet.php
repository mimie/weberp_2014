...
/* $Id: GLBalanceSheet.php 5239 2012-04-12 07:43:22Z vvs2012 $*/
...
//die($SQL);
	...
	$SQL = "SELECT accountgroups.sectioninaccounts,
			...
			chartmaster.accountname,
			chartmaster.glacode,
			Sum(CASE WHEN chartdetails.period='" . $_POST['BalancePeriodEnd'] . "' THEN chartdetails.actual ELSE 0 END) AS balancecfwd,
			Sum(CASE WHEN chartdetails.period='" . ($_POST['BalancePeriodEnd'] - 12) . "' THEN chartdetails.actual ELSE 0 END) AS lybalancecfwd,
			Sum(CASE WHEN chartdetails.period='" . ($_POST['BalancePeriodEnd'] - 1) . "' THEN chartdetails.actual ELSE 0 END) AS lmbalancecfwd
		FROM chartmaster INNER JOIN accountgroups
		ON chartmaster.group_ = accountgroups.groupname INNER JOIN chartdetails
		ON chartmaster.accountcode= chartdetails.accountcode
		WHERE accountgroups.pandl=0
...
//die($SQL);
	...
	if ($_POST['Detail']=='Detailed'){
		$temp1=strtotime($BalanceDate);
		$temp2=strtotime($BalanceDate."-1 months");
		$cbd=date('F',$temp1).date('d',$temp1);
		$lbd=date('F',$temp2).date('d',$temp2);
		$TableHeader = '<tr>
							<th>'._('Account').'</th>
	...
	while ($myrow=DB_fetch_array($AccountsResult)) {
		$AccountBalance = $myrow['balancecfwd'];
		$LYAccountBalance = $myrow['lybalancecfwd'];
		$LMAccountBalance=$myrow['lmbalancecfwd'];
		//echo 'here'.$LMAccountBalance;
		if ($myrow['accountcode'] == $RetainedEarningsAct){
			$AccountBalance += $AccumProfitRow['accumprofitbfwd'];
			$LYAccountBalance += $AccumProfitRow['lyaccumprofitbfwd'];
		}
			...
			if ($myrow['parentgroupname']!=$ActGrp){
				while ($myrow['groupname']!=$ParentGroups[$Level] AND $Level>0){
					...
					printf('<tr>
						...
						locale_number_format($LMAccountBalance[$Level],$_SESSION['CompanyRecord']['decimalplaces']),
		...
		if ($_POST['Detail']=='Detailed'){
			...
			$ActEnquiryURL = '<a href="' . $rootpath . '/GLAccountInquiry.php?Period=' . $_POST['BalancePeriodEnd'] . '&amp;Account=' . $myrow['accountcode'] . '">' . $myrow['glacode'] . '</a>';
			...
			printf('<td>%s</td>
					...
					locale_number_format($LMAccountBalance,$_SESSION['CompanyRecord']['decimalplaces']),
...
echo '<form method="post" action="' .$rootpath. '/print/printBSheet.php?ref='. $_POST['BalancePeriodEnd'].'&amp;lbd='.$lbd.'&amp;cbd='.$cbd.'" target="blank">';
    echo '<div>';
        echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';

echo '</div>';
        echo '<br /><div class="centre"><input type="submit" name="SelectADifferentPeriod" value="'._('Print').'" /></div>';
	echo '<input type="hidden" name="num2" value="">';
echo '</div></form>';


echo '<form method="post" action="' .$rootpath. '/print/printBSheet.php?ref='. $_POST['BalancePeriodEnd'].'&amp;lbd='.$lbd.'&amp;cbd='.$cbd.'&amp;export=1" target="blank">';
    echo '<div>';
        echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';

echo '</div>';
        echo '<br /><div class="centre"><input type="submit" name="Export" value="'._('Export To Excel').'" /></div>';
        echo '<input type="hidden" name="num2" value="">';
echo '</div></form>';
