...
/* $Id: GLJournal.php 5239 2012-04-12 07:43:22Z vvs2012 $*/
...
*/
	...
	$d3=$TransNo;
	$myCtr=strlen($TransNo);
	 while($myCtr<4){
	 $d3='0'.$d3;
	 $myCtr++;
	}
	$jvNum=date("Y-m-",strtotime($_SESSION['JournalDetail']->JnlDate)).$d3;
	#echo $_POST['Particulars'];
	#$die($jvNum);
	...
	prnMsg(_('Journal').' ' . $jvNum . ' '._('has been successfully entered'),'success');
	echo '<br><br><table border="1"><tr><th><b>Process</b></th></tr><tr><th>';
	echo '<a href="'.$rootpath.'/print/printJV.php?vouchNum='.$jvNum.'&amp;Uname='.$Uname.'" target="blank">Print Journal Voucher</a>';
	echo '</th></tr>';
	...
	echo '<tr><th>
	<a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?NewJournal=Yes">'._('Enter Another General Ledger Journal').'</a></th></tr><table><br><br>';
...
echo '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Particulars='.$Particulars.'" method="post" id="form">';
echo '<div>';
...
$jvNum=LookNextTransNo(0,$db);
echo '<table>
        <tr>
        <td colspan="5"><table class="selection">
						<tr>
							<td>'._('Date to Process Journal') . ':</td>
							<td><input type="text" class="date" 
										alt="' . $_SESSION['DefaultDateFormat'] . '" 
										name="JournalProcessDate" 
										maxlength="10" 
										size="11"
										value="' . $_SESSION['JournalDetail']->JnlDate . '" />
							</td>
							<td>' . _('Type') . ':</td>
...
$vouchNum=$_POST['JournalProcessDate'];
...
echo '<tr>
    <th colspan="3">
    <div class="centre"><h2>' . _('Journal Details') . '</h2></div>
    </th>
  </tr>';


/*echo '<tr>
    <th>' . _('Voucher Number:') . '</th>
    <td colspan="2"><input type="text" id="VoucherNumber" name="VoucherNumber" maxlength="100" size="70" value="' . $_POST['VoucherNumber'] . '" /></td>
  </tr>';*/
...
echo '</tr>
	...
	<tr>
		...
		<td colspan="2"><input type="text" name="GLNarrative" maxlength="50" size="50" value="' . $_POST['GLNarrative'] . '" /></td>
	</tr>
	</table>
...
#die(print_r($JournalItem));
	...
	echo '<td>' . $JournalItem->tag . ' - ' . $TagDescription . '</td>
		<td>' . $JournalItem->assetid . ' - ' . $JournalItem->GLActName . '</td>';

	
$myGLActName=$myGLActName. $JournalItem->GLActName.'*';
$myGLActCode=$myGLActCode. $JournalItem->assetid.'*';
	...
	if ($JournalItem->Amount>0) {
		...
		$myDebit=$myDebit.$JournalItem->Amount.'*';
		$myCredit=$myCredit.'0.00'.'*';
	...
	$myGLNarative=$myGLNarative. $JournalItem->Narrative.'>';
