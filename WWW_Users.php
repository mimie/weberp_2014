...
/* $Id: WWW_Users.php 5658 2012-09-12 06:30:58Z icedlava $*/
...
// User Identity
$sql="SELECT identity_name, 
							display_module
			FROM www_users_identity";
$ctr=0;
$UIdentity=array();
$result= DB_query($sql,$db);
while($row=DB_fetch_row($result)){
	$UIdentity[$ctr]['name']=$row[0];
	$UIdentity[$ctr]['module']=$row[1];
	$ctr++;
}
...
/*SelectedUser could also exist if submit had not been clicked this code would not run in this case cos submit is false of course  see the delete code below*/
		...
		$sql = "UPDATE www_users SET realname='" . $_POST['RealName'] . "',
						...
						modulesallowed='" . $_POST['UserIdentity']  . "',
...
//echo '<input type="submit" value="Add User" name="submit"></input>';
...
echo '<tr>
                <td>' . _('Re-Type Password') . ':</td>
                <td><input type="password" name="RePassword" size="22" maxlength="20" value="' . $_POST['RePassword'] . '" /></td>
        </tr>';
...
//New User Identity
echo '<tr>
		<td>' . _('User Identity') . ':</td>
		<td><select name="UserIdentity">';

$ctr1=0;
while($ctr1<=7){

	echo "<option value='".$UIdentity[$ctr1]['module']."'>".$UIdentity[$ctr1]['name']."</option>";
	$ctr1++;
...
//---Combo Box for modules

/**foreach($ModuleList as $ModuleName){

	echo '<tr>
			<td>' . _('Display') . ' ' . $ModuleName . ' ' . _('module') . ': </td>
			<td><select name="Module_' . $i . '"> Here';
...
echo '</table>
	<br />
	<div class="centre">
		<input type="submit" name="submit" value="' . _('Save Information') . '" />
	</div>
    </div>
	</form>';
