...
/* $Id: CustomerReceipt.php 5620 2012-09-03 10:34:54Z daintree $ */
...
//dito print
		...
		if ($ReceiptItem->GLCode ==''){
			...
			echo '<td><a target="_blank"  href="' . $rootpath . '/print/printOR.php?BatchNumber=' . $_SESSION['ReceiptBatch']->BatchNo. 
																						'&ReceiptNumber='.$receiptNumber.
																						'&amp;CustomerName='.$ReceiptItem->CustomerName.
																						'&amp;myAmount='.$ReceiptItem->Amount .
																						'&amp;myDate='.$_SESSION['ReceiptBatch']->DateBanked.
																						'&amp;myAddress='.$myAddress.
																						'&amp;myNarative='.$ReceiptItem->Narrative.
																						'&amp;formPayment='. $_POST['ReceiptType'].
			'">'._('Print a Customer Receipt').'</a></td></tr>';
	...
	if ($_SESSION['CompanyRecord']['gllink_debtors']==1){ /* then enter GLTrans records for discount, bank and debtors */
		...
		$myBill=explode(":",$ReceiptItem->Narrative);
		//echo $myBill[1];
		//die();

		$sql2="SELECT fee_amount,amount_paid FROM billing_membership WHERE billing_no='".$myBill[1]."'";
		$result2=mysql_query($sql2);
		$row2=mysql_fetch_array($result2);
		$myAmt=$row2['fee_amount']-$row2['amount_paid'];
		if($myAmt<=$BatchDebtorTotal){
			$sql2="UPDATE billing_membership SET
				amount_paid=amount_paid+".$BatchDebtorTotal.",
				paid_bill=1
				WHERE billing_no='".$myBill[1]."'
				";
		}else{
		$sql2="UPDATE billing_membership SET
			amount_paid=amount_paid+".$BatchDebtorTotal."
...
elseif(isset($_POST['Add'])){
  ...
  //header("addCustomer.php?searchText=$searchText");
}
...
//action koh
echo '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Type='.$_GET['Type'] . '&amp;receiptNumber='.$_POST['receiptNumber'].'" method="post" id="form1">';
echo '<div>';
...
echo '<tr>
	...
	<tr>
                <td>' . _('DCT Number') . ':</td>
                <td><input tabindex="7" type="text" name="DCTNum" maxlength="50" size="52" value="'.$dct.'" /></td>
        </tr>


<tr>
    <td>' . _('Receipt Number') . ':</td>
    <td><input tabindex="7" type="text" name="receiptNumber" maxlength="50" size="52" value="'.$receiptNumber.'" /></td>
  </tr>

<tr>
	<td>Receipt Class:</td>
	<td>
		<select tabindex="8" name="ReceiptClass">';
		if (isset($_POST['ReceiptClass']) AND $_POST['ReceiptClass']!=''){
		echo '<option selected="selected" value"'.$_POST['ReceiptClass'].'">'.$_POST['ReceiptClass'].'</option>';
		}
		echo'
		<option value="OR-Official Receipt">OR-Official Receipt</option>
		<option value="AR-Acknowledgment Receipt">AR-Acknowledgment Receipt</option>
		</select>
	</td>
...
/*a customer is selected  */
	...
	if(isset($_POST['Inquire'])){
		include('connectDb.php');
		$contId=explode('P',$_POST['CustomerID']);
		...
		while($row2=mysql_fetch_array($result2)){
			//echo $row2['fee_amount']-$row2['amount_paid'];
			$myAmt=$row2['fee_amount']-$row2['amount_paid'];
			echo'<tr>
			<td><input type="submit" value="'.$row2['billing_no'].'" name="myBill"/></td>
			<td>'.$row2['year'].' Membership</td>
			<td class="number">'.number_format($row2['vat'],2).'</td>
			<td class="number">'.number_format($row2['subtotal'],2).'</td>
			<td class="number">'.number_format($myAmt,2).'</td>
			<td>'.$row2['bill_date'].'</td>
			<input type="hidden" value="'.$row2['year'].' Membership" name="eventName"/>
			<input type="hidden" value="'.$myAmt.'" name="amount"/>
		</tr>';
		}
		...
		while($row2=mysql_fetch_array($result2)){
                        ...
                        <td class="number">'.number_format($row2['vat'],2).'</td>
                        <td class="number">'.number_format($row2['subtotal'],2).'</td>
                        <td class="number">'.number_format($row2['fee_amount'],2).'</td>
                        <td>'.$row2['bill_date'].'</td>
			<input type="hidden" value="'.$row2['year'].' Membership" name="eventName"/>
                        <input type="hidden" value="'.$row2['fee_amount'].'" name="amount"/>
                </tr>';
                }
		echo '</table>';
	}

	if(isset($_POST['myBill'])){
		//echo $_POST['eventName'];	
		$amt=$_POST['amount'];
		$_POST['Amount']=$amt;
		$nar='Reference number:'.$_POST['myBill'];

	}


	echo '<table class="selection">';
	echo '<tr><th colspan="2"><input type="submit" name="Inquire" value="Inquire Billing"></th></tr>';
...
if (((isset($_SESSION['CustomerRecord'])
	...
	//echo '<tr><th colspan="2"><a href="#">Inquire Membership bill</a></th></tr>';
	echo '<tr>
...
// End select tag
        ...
        if (DB_num_rows($result)==0){
                ...
                while ($myrow=DB_fetch_array($result)){
                        if ($_POST['GLCode']==$myrow['accountcode']){
                                echo '<option selected="selected" value="' . $myrow['accountcode'] . '">' . $myrow['glacode'] . ' - ' . $myrow['accountname'] . '</option>';
                        } else {
                        echo '<option value="' . $myrow['accountcode'] . '">' . $myrow['glacode'] . ' - ' . $myrow['accountname'] . '</option>';
                        }
                }
                echo '</select></td>
                        </tr>';
...
//		<td><input tabindex="13" type="text" name="Narrative" maxlength="30" size="32" value="' . $_POST['Narrative'] . '" /></td></tr>';
	...
	echo '<p class="page_title_text">
		...
		<div class="centre">
			...
			<br/>
			<br/>
                        <a href="addCustomer.php?searchText='.$searchText.'">
                        <input tabindex="13" type="button" name="Add" value="' . _('Add Customer') . '" />
                        </a>
                        <br/>
                        <br/>
