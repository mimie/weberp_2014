...
/* $Id: Payments.php 5622 2012-09-04 04:29:17Z tehonu $*/
...
#die('Here'.$_SESSION['PaymentDetail'.$identifier]->SupplierID);
...
if (isset($_POST['CheckDate']) AND $_POST['CheckDate']!=''){
	$checkdate=$_POST['CheckDate'];
}
...
if (isset($_POST['CommitBatch'])){
	...
	if ((!isset($_POST['ChequePrinted']))
		...
		echo '<form method="post" action="' . htmlspecialchars($_SERVER['PHP_SELF'] . '?identifier=' . $identifier.'&cv='.$_POST['VouchNum'].'&supplierID='.$_GET['supplierID'].'&invoice='.$_POST['Invoice']) . '&checkdate='.$_GET['checkdate'].'">';
		echo '<div>';

		echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
		echo _('Are you sure you want to process this payment') . '?
			<br />
			<br />
		...
		echo '<br />Payment amount = ' . number_format($_SESSION['PaymentDetail' . $identifier]->Amount,2);
		echo '</div>
			</form>';
...
#die('Here'.$_SESSION['PaymentDetail'.$identifier]->SupplierID);
					...
					$SQL= $SQL . "VALUES (1,
						...
						'" . $PaymentItem->Tag . "',
						'".$_GET['cv']."',
						'". FormatDateForSQL($_GET['checkdate']) ."',
						'".$_GET['supplierID']."',
						'".$_GET['invoice']."'
							)";
		...
		DB_Txn_Commit($db);
		prnMsg(_('Payment') . ' ' . $_GET['cv'] . ' ' . _('has been successfully entered'),'success');
		$aa=$_GET['cv'];
		echo '<br><br><a href="' . $rootpath . '/print/printCheckBPI.php?voucherNum='.$aa.'" target="blank">Print BPI Check</a>';
		echo '<br><br><a href="' . $rootpath . '/print/printCheckBDO.php?voucherNum='.$aa.'" target="blank">Print BDO Check</a>';
		echo '<br><br><a href="' . $rootpath . '/print/printCheckBIR.php?voucherNum='.$aa.'" target="blank">Print BIR Check</a>';
...
if ($_SESSION['PaymentDetail' . $identifier]->SupplierID==''){
	...
	echo '<tr><td>Supplier Name:</td><td><input type="text" size="55" name="suppName" value="'.$suppName.'"></td></tr>';
...
if (!isset($_POST['VouchNum']) OR $_POST['VouchNum']=='') {
	//$_POST['VouchNum']='';
	$VouchNum = LookNextTransNo( 1, $db);
	$myLen=strlen($VouchNum);
	while($myLen<5){
		$VouchNum="0".$VouchNum;
		$myLen++;
	}
...
echo '<tr>
                <td>' . _('Voucher Number') . ':</td>
		
                <td><input type="text" name="VouchNumDisplay"  size="17" value="' . $VouchNum . '" disabled /> 
		    <input type="hidden" name="VouchNum" size="17" value="'.$VouchNum.'" />
		</td>
        </tr>';
